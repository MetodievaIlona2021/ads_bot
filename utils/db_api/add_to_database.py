...
async def add_asks():
    await add_ask(
        name='Как вы находите респондентов?',
        text='У нас собственная база респондентов 54 тыс, собранная за 10 лет с помощью различных методов: '
             'рекрут уличный и в ТЦ, соц. сети, снежный ком. База постоянно пополняется вышеуказанными методами. '
             'Рекрутируем по базе, снежным комом, а так же с помощью соц. сетей и мобильного приложения.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Как быстро сможете подобрать респондентов?',
        text='Рекрутинг потребителей товаров повседневного спроса и услуг составит в большинстве случаев по '
             'срокам 1 рабочий день, ЦА труднодоступная (эксперты, высокодоходная аудитория и т.д.) - 2 рабочих дня '
             'для рекрутинга первых респондентов.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Из чего складывается стоимость рекрутинга?',
        text='В стоимость входит: вознаграждение респондентам, услуга рекрутинга (если рекрут с дополнительными '
             'критериями, то это усложняет поиск и, соответственно цена будет выше, так как наша основная цель '
             'выполнить проект качественно), комиссия на выплату вознаграждения.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Как проверяете подходит ли респондент под критерии? Есть ли скринирующая анкета, как составляете '
             'вопросы? Есть ли общение по телефону голосом? Запрашиваете ли фото/документы подтверждающие '
             'определенные характеристики?',
        text='Мы тщательно скринируем респондентов, каждый респондент проходит двойной скрининг, в том числе и '
             'телефонный, включающий в себя дополнительные вопросы, касаемые использования продукта. '
             'Во время скрининга ведем аудиозапись. Так же от респондентов запрашивается любое подтверждение '
             'использования продукта (документы/фото/скрины), для исключения возможности скачивания фото из '
             'интернета или фотошопа мы просим выполнить определенные действия, например, сделать скриншот на '
             'компьютере с определенной открытой вкладкой или прислать фото лекарства и рядом положить бумажку с '
             'текстом "на опрос". Скрининговую анкету обычно направляет клиент, но если ее нет, то мы самостоятельно '
             'ее составляем на основе критериев и направляем клиенту на одобрение.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Рекрутируете ли вы респондентов в регионах?',
        text='В основном мы специализируемся на рекрутинге респондентов в Москве и Московской области, частично '
             'выполняем рекрут в Санкт-Петербурге и области. В данный момент мы самостоятельно активно развиваем '
             'направление рекрутинга в регионах, без привлечения партнеров, так как это несет дополнительные риски '
             'по качеству. В регионах рекрутируем потребителей товаров повседневного спроса, онлайн опросы. '
             'Одни из последних выполненных проектов в регионах были пользователи кредитов за последние 6 мес. '
             '(6 чел) и те, кто слушают музыку на различных ресурсах ( Яндекс.Музыка, Спотифай, ВК музыка, Apple '
             'music, СберЗвук, ( 5 чел.).',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Как вы контролируете приход респондентов?',
        text='Мы контролируем респондентов за день до исследования вечером, а также за 3 часа до исследования. '
             'При контроле осуществляем повторный скрининг респондентов и производим аудиозапись скрининга.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Что происходит, если респондент не пришел на исследование или в начале интервью выясняется, '
             'что респондент не подходит?',
        text='В данном случае мы бесплатно заменяем респондента на удобные для Вас даты и время. По возможности, '
             'если проект не сложный, можем осуществить замену в день участия. Мы не делаем двойной запас для '
             'оптимизации Вашего бюджета. Если заранее были учтены и назначены запасные респонденты, то мы не '
             'берем оплату за "отмененного" респондента, а подтверждаем участие респондента из запаса.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Как Вы организуете дистанционное участие респондентов, например, в удаленном интервью по ZOOM?',
        text='Клиент заранее создает конференцию и направляет нам ссылки на каждого респондента, мы в свою очередь '
             'высылаем эти ссылки респондентам. При рекрутинге респондентов на удаленные исследования мы заранее '
             'уточняем у респондентов наличие технической возможности для участия (необходимое устройство для '
             'подключения, камера, микрофон и т.д.), а также сообщаем о необходимости стабильного интернет соединения '
             'и тихого места. Далее, в день участия, за 2 часа до исследования мы самостоятельно выходим на связь с '
             'респондентом в той же программе, в которой и будет проходить исследование и проверяем скорость '
             'интернета ( с помощью специальной программы), работу камеры, микрофона, умение включать демонстрацию '
             'экрана, проводим скрининг респондента.. Во время проверки связи мы делаем скриншот и осуществляем '
             'аудиозапись скрининга, По Вашему желанию мы можем во время проверки респондента записать видео и '
             'попросить респондента коротко рассказать о себе, о своих увлечениях, а также попросить '
             'продемонстрировать наличие необходимого для исследования продукта, например, респондент должен '
             'пользоваться антивирусом, мы попросим его включить демонстрацию экрана и продемонстрировать свой '
             'антивирус. Такое видео мы направим Вам до интервью, чтобы Вы могли заранее ознакомиться с респондентом.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Берете ли Вы функцию выплаты вознаграждения респондентам на себя?',
        text='Да, мы организуем выплату вознаграждений респондентам. Выплаты происходят участникам на карту в день '
             'участия вечером или на номер мобильного телефона по завершении исследования. Все респонденты заранее '
             'предупреждаются о сроках и методах выплат, а также заранее передают свои данные для оплаты.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Как проверяете респондентов на профессиональность?',
        text='Мы ведем собственную базу участников и черный список респондентов, проверяем респондентов на наличие в '
             'данных базах, а так же уточняем при скрининге сколько раз и когда респондент до этого принял участие + '
             'на какую тему был прошлый опрос, Помимо этого мы не приглашаем респондентов, которые при скрининге '
             'сухо отвечают на вопросы или наоборот чрезмерно общительны.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Проверяете ли Вы респондентов по базе ОИРОМ?',
        text='К сожалению, у нас нет доступа к базе маркетинговых агентств ОИРОМ, но если Вы предоставляете доступ к '
             'данной базе или можете проверить по ней самостоятельно, то мы выполняем рекрутинг на такие проекты.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='Рекрутируете ли Вы респондентов по базе клиента?',
        text='Мы не осуществляем рекрутинг респондентов по базе клиента, так как холодная обработка базы, к '
             'сожалению, как показывает практика, не дает результата или он совсем минимальный, а трудозатраты в '
             'итоге большие, так же такие респонденты менее отвественны, они чаще отказываются в последний момент, '
             'не выходят на связь и т.д.. Более подробно мы описали в нашей статье: https://clck.ru/SPqB9 '
             'Мы можем выполнить рекрутинг по базе клиента, если есть возможность заранее таким респондентам '
             'разослать со стороны клиента приглашения к участию на емейл и направить нам уже согласившихся '
             'респондентов, таким образом они будут более лояльны к участию. Так мы работали, например, с '
             'Лабораторией Касперского и результат от такой обработки базы очень хороший.',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
    await add_ask(
        name='В каком формате и какую информацию предоставляете клиенту о предстоящих встречах?',
        text='Мы создаем гугл табличку (статус), в которую вносим скрининговые вопросы и ответы респондентов на них. '
             'В режиме реального времени Вы будете видеть процесс рекрутинга: Пример статуса: '
             'https://docs.google.com/spreadsheets/d/1ogh3Az0-vvzmsiqCP2sXHzr1fdoTJwvSy6Emk55-5Dw/edit?usp=drivesdk',
        thumb_url='https://static.wixstatic.com/media/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png/v1/fit/w_2500,h_1330,al_c/4cd91a_7c02e50f680e47e1910186ffcc0c1e7d.png',
        category='asks'
    ),
...
# loop.run_until_complete(add_asks())
